...
const Comparator = () => {
    ...
    const handleCategorySelect = (category) => {
        setFormData((prev) => ({
            ...prev,
            category: category, // Normalize for backend
        }));
    };

    const handleInputSizeSelect = (inputSize) => {
        setFormData((prev) => ({ ...prev, input_size: inputSize }));
    };
    ...
    return (
        <div className="scrollbar-hide overflow-auto h-screen bg-base-200">
            ...
            <div className="flex flex-col items-center justify-center">
                <div className="card w-full h-fit max-w-270">
                    <div className="card-body bg-base-300 rounded-xl">
                        <h3 className="text-xl font-semibold">Algorithm Category</h3>
                        <div className="flex flex-row items-center justify-between gap-2">
                            <div className="dropdown flex-1">
                                <div tabIndex={0} role="button" className="btn btn-outline rounded-lg w-full justify-between">
                                    {formData.category || 'Select Algorithm'}
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m7 10l5 5m0 0l5-5"/></svg>
                                </div>
                                <ul tabIndex={0} className="dropdown-content menu bg-base-100 border-1 border-white  rounded-box z-1 p-2 shadow-sm w-full mt-1">
                                    {categories.map((size) => (
                                        <li key={size}>
                                            <a onClick={() => handleCategorySelect(size)}>{size}</a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            <div className="dropdown flex-1">
                                <div tabIndex={0} role="button" className="btn btn-outline rounded-lg w-full justify-between">
                                    {formData.input_size || 'Input Size Range'}
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m7 10l5 5m0 0l5-5"/></svg>
                                </div>
                                <ul tabIndex={0} className="dropdown-content menu bg-base-100 border-1 border-white rounded-box z-1 p-2 shadow-sm w-full mt-1">
                                    {inputSizes.map((size) => (
                                        <li key={size}>
                                            <a onClick={() => handleInputSizeSelect(size)}>{size}</a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            <button onClick={handleCompare} disabled={isLoading} className="btn rounded-lg flex-1">Compare Algorithms</button>
                        </div>
                    </div>
                </div>
                ...
                {!result ? (
                    <div className="card w-full h-fit max-w-270 mt-7.5">
                        <div className="card-body bg-base-300 rounded-xl h-fit">
                            <div className="flex flex-col items-center justify-between">
                                {isLoading ? (
                                    ...
                                    <div className="flex flex-col items-center justify-between">
                                        ...
                                        <p className="text-sm mb-6">Select algorithm categories and input size range above to start comparing algorithms.</p>
                                        <div className="flex flex-row items-center justify-center gap-4">
                                            <div className="flex flex-row items-center justify-center px-4 py-2 bg-accent-content rounded-lg gap-2">
                    ...
                    <div className="flex flex-col items-center justify-center w-full">
                        <div className="card w-full h-fit max-w-270 mt-7.5">
                            <div className="card-body bg-base-300 rounded-xl">
                                <h3 className="text-xl font-semibold">Algorithm Comparison</h3>
                        ...
                        {result.recommendations.map((rec, index) => (
                            <div key={index} className="card w-full h-fit max-w-270 mt-7.5">
                                <div className="card-body bg-base-300 rounded-xl">
                                    ...
                                    <div className="flex flex-col items-start justify-center bg-base-100 rounded-lg px-4 py-2.5">
                                        <h4 className="font-bold text-sm">Recommended Algorithm: {rec.recommendation}</h4>
                                        <p className="font-medium text-sm">{rec.insights}</p>
                                    </div>
                                    <div className="flex flex-row items-stretch justify-center gap-2 w-full h-full">
                                        <div className="flex flex-col items-start justify-start w-full bg-base-100 rounded-lg px-4 py-2.5">
                                            <h4 className="font-bold text-sm">Performance Tips</h4>
                                        ...
                                        <div className="flex flex-col items-start justify-center w-full bg-base-100 rounded-lg px-4 py-2.5">
                                            <h4 className="font-bold text-sm">Trade-offs</h4>
                                            <ul className="list-disc list-inside">
