...
const Binary = () => {
    ...
    const executeStep = (step) => {
        if (step.type === "checking") {
            ...
            if (step.type === "search_left") setPseudocodeHighlight(10);
            if (step.type === "search_right") setPseudocodeHighlight(8);
        ...
        <div className="flex flex-col scrollbar-hide overflow-auto h-screen bg-base-200 relative">
                                        ...
                                        <div className="card-body p-6">
                                            ...
                                            <div className="mt-4 p-4 bg-info/10 rounded-xl border-l-4 border-info">
                                                <p className="text-sm text-base-content/80 leading-relaxed">
                                                    ...
                                                    resulting in logarithmic time complexity with constant space usage. Requires a sorted array.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div className="flex-grow flex flex-col items-center justify-start mt-6 px-4 overflow-auto">
                    ...
                    <div className="mb-4">
                        <div className="badge badge-primary badge-lg">
                            {currentStep.type === "checking"
                                ? `Checking middle index ${currentStep.index}: value = ${data[currentStep.index]}, range [${currentStep.left}...${currentStep.right}]`
                                : currentStep.type === "search_left"
                                    ? `Searching left half [${currentStep.left}...${currentStep.right}]`
                                    : currentStep.type === "search_right"
                                        ? `Searching right half [${currentStep.left}...${currentStep.right}]`
                ...
                <div className="w-full max-w-6xl mb-6">
                    <h3 className="text-lg font-bold mb-3 text-center">Sorted Array (Binary Search requires sorted data)</h3>
                    <div className="flex flex-wrap justify-center gap-2">
                        {data.map((num, idx) => {
                            const isHighlighted = highlightedIndices.includes(idx);
                            const isMid = midIndex === idx;
                            const isInRange = searchRange && idx >= searchRange.left && idx <= searchRange.right;
                            const isOutOfRange = searchRange && (idx < searchRange.left || idx > searchRange.right);
                            ...
                            } else if (isInRange && !isOutOfRange) {
                                bgColor = 'bg-blue-200';
                            ...
                            return (
                                <div
                                    ...
                                    className={`
                                        ...
                                        ${isHighlighted || isFound || isMid ? 'scale-110 shadow-lg' : ''}
                                    `}
                                >
                ...
                {searchRange && (
                    <div className="mb-4 flex gap-6">
                        <div className="flex items-center gap-2">
                            <span className="font-bold text-green-500 text-lg">L</span>
                            <span className="text-sm">= {searchRange.left}</span>
                        </div>
                        {midIndex !== null && (
                            <div className="flex items-center gap-2">
                                <span className="font-bold text-yellow-500 text-lg">M</span>
                                <span className="text-sm">= {midIndex}</span>
                            </div>
                        )}
                        <div className="flex items-center gap-2">
                            <span className="font-bold text-purple-500 text-lg">R</span>
                            <span className="text-sm">= {searchRange.right}</span>
                        </div>
                    </div>
                )}
                ...
                <div className="flex flex-wrap gap-4 justify-center mb-6">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">Middle (M)</span>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="w-8 h-8 bg-gray-200 border-2 border-gray-400 rounded opacity-40"></div>
                            ...
                            <div className="card-body p-3 w-78">
                                ...
                                <div className="bg-base-200 rounded-lg p-2 font-mono text-xs space-y-0.5">
                                    <div className={`px-2 py-1 rounded transition-all ${pseudocodeHighlight === 1 ? 'bg-primary/20 border-l-2 border-primary' : ''}`}>
                                        <span className="text-primary font-bold">function</span> binarySearch(array, target):
                                    </div>
                                    ...
                                    <div className={`px-2 py-1 rounded transition-all ml-2 ${pseudocodeHighlight === 3 ? 'bg-info/20 border-l-2 border-info' : ''}`}>
                                        <span className="text-info font-bold">while</span> left &lt;= right:
                                    </div>
                                    <div className={`px-2 py-1 rounded transition-all ml-4 ${pseudocodeHighlight === 4 ? 'bg-warning/20 border-l-2 border-warning' : ''}`}>
                                        mid = (left + right) / 2
                                    </div>
                                    <div className={`px-2 py-1 rounded transition-all ml-4 ${pseudocodeHighlight === 5 ? 'bg-accent/20 border-l-2 border-accent' : ''}`}>
                                        <span className="text-accent font-bold">if</span> array[mid] == target:
                                    </div>
                                    <div className={`px-2 py-1 rounded transition-all ml-6 ${pseudocodeHighlight === 6 ? 'bg-success/20 border-l-2 border-success' : ''}`}>
                                        <span className="text-primary font-bold">return</span> mid
                                    </div>
                                    <div className={`px-2 py-1 rounded transition-all ml-4 ${pseudocodeHighlight === 7 ? 'bg-warning/20 border-l-2 border-warning' : ''}`}>
                                        <span className="text-warning font-bold">else if</span> array[mid] &lt; target:
                                    </div>
                                    <div className={`px-2 py-1 rounded transition-all ml-6 ${pseudocodeHighlight === 8 ? 'bg-warning/20 border-l-2 border-warning' : ''}`}>
                                        left = mid + 1
                                    </div>
                                    <div className={`px-2 py-1 rounded transition-all ml-4 ${pseudocodeHighlight === 9 ? 'bg-warning/20 border-l-2 border-warning' : ''}`}>
                                    ...
                                    <div className={`px-2 py-1 rounded transition-all ml-2 ${pseudocodeHighlight === 11 ? 'bg-error/20 border-l-2 border-error' : ''}`}>
